...
icon = os.path.join(RUTA_RECURSOS, 'calculadora.png')
DB_PATH = os.path.join(RUTA_RECURSOS, 'productos.db')
...
def init_local_db():
    ...
    # ------------------------------------------------------------
    # Si quieres migrar automáticamente, podrías DROP la tabla antigua.
    # ADVERTENCIA: esto borra los datos actuales. Alternativamente,
    # pide al usuario que elimine productos.db para recrear con este esquema.
    # ------------------------------------------------------------

    # Nueva tabla: 'id' es PK AUTOINCREMENTAL, 'codigo' ya no es UNIQUE
    cur.execute('''
...
def load_local_products():
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    # Selecciona sólo codigo y descripcion; el id queda oculto
    cur.execute("SELECT codigo, descripcion FROM productos ORDER BY id")
    productos = cur.fetchall()
    con.close()
    return [c for c, _ in productos], [d for _, d in productos]

def save_local_products(items):
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    # 1) Vaciamos la tabla (se borran también los id antiguos; se reenumerarán)
    cur.execute("DELETE FROM productos")
    # 2) Insertamos todos los ítems (SQLite asigna id automáticamente)
    cur.executemany(
        "INSERT INTO productos(codigo, descripcion) VALUES (?,?)",
        items
    )
    # 3) Actualizamos last_count
    cur.execute(
...
def obtener_datos_productos():
    ...
    # Cargo y devuelvo inmediatamente los datos locales
    local_codigos, local_descripciones = load_local_products()
    local_count = get_local_count()
    print(f"[DEBUG] Local count = {local_count}, loaded {len(local_codigos)} rows")
    ...
    try:
        ...
        # 3) Conexión rápida a Mongo + ping
        client = MongoClient(MONGO_URI,
            serverSelectionTimeoutMS=500,   # máximo 0.5 s para seleccionar servidor
            connectTimeoutMS=500,           # máximo 0.5 s para establecer socket
            socketTimeoutMS=500,            # máximo 0.5 s por operación de socket
            connect=False                   # no conecta hasta la primera operación
        )
        db   = client['Royal']
        coll = db['productos']

        # 2) count_documents con maxTimeMS para fallo rápido
        remote_count = coll.count_documents({})
        print(f"[DEBUG] Remote count = {remote_count}")

        if remote_count != local_count:
            print("[DEBUG] Conteos distintos, descargando colección completa…")
            # Hacemos un list() para poder medirlo antes de save_local_products
            docs = coll.find(
                {},
                {"CODIGO":1, "DESCRIPCION DEL PRODUCTO":1, "_id":0},
                max_time_ms=500
            )
            docs_list = list(docs)
            print(f"[DEBUG] find() trajo {len(docs_list)} docs")

            items = []
            for p in docs_list:
                raw = str(p.get("CODIGO",""))
                codigo = raw.strip() if raw.strip() != "" else ""    # dejamos blanks tal cual
                desc   = str(p.get("DESCRIPCION DEL PRODUCTO","")).strip()
                items.append((codigo, desc))

            if items:
                save_local_products(items)
                print(f"[DEBUG] Guardados {len(items)} registros en SQLite")
            else:
                print("[WARN] ¡No se obtuvo ningún documento útil de Mongo!")
    except Exception as e:
        # este except ahora englobará sólo errores de conexión / count
        print(f"[OFFLINE] No puedo conectar a MongoDB o count_documents falló: {e!r}")

    # Finalmente, devuelvo *otra vez* lo que quedó en SQLite
    cods, descs = load_local_products()
    print(f"[DEBUG] Devolviendo {len(cods)} productos desde local")
    return cods, descs
...
Window.set_icon(os.path.join(RUTA_RECURSOS, 'calculadora.png'))
...
class MiWidget(Widget):
    def __init__(self, **kwargs):
        ...
        # 1) Crear el Spinner y guardar tamaños “base”
        self.cuotas_spinner = Spinner(
            text='',
            values = ['6 Meses', '12 Meses', '18 Meses', '24 Meses', '26 Meses'],
            size_hint=(None, None)
        )
        ...
        self.btn_config = Button(text="⚙", font_name=cargar_archivo('DejaVuSans.ttf'), font_size=30, size_hint=(None, None), size=(30, 30))
