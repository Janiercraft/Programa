...
def init_local_db():
    ...
    # 1) Crear tabla productos (con precio_compra)
    cur.execute('''
    ...
    # 2) Migración automática: si falta precio_compra, lo añadimos
    ...
    # 3) Crear tabla metadata y claves necesarias
    cur.execute('''
...
def load_local_products():
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("SELECT codigo, descripcion, precio_compra FROM productos ORDER BY id")
    productos = cur.fetchall()
    con.close()
    cods    = [c for c,_,_ in productos]
    descs   = [d for _,d,_ in productos]
    precios = [p for _,_,p in productos]
    return cods, descs, precios

def save_local_products(items):
    """
    items: lista de tuplas (codigo, descripcion, precio_compra)
    """
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("DELETE FROM productos")
    cur.executemany(
        "INSERT INTO productos(codigo, descripcion, precio_compra) VALUES (?,?,?)",
        items
    )
    cur.execute(
...
def hash_remote_products(coll):
    """
    MD5 de [(codigo, descripcion, precio), ...] ordenado.
    Campo exacto de Mongo: "PRECIO DE\nCOMPRA"
    """
    docs = list(coll.find(
        {},
        {
    ...
    for p in docs:
        ...
        precio = float(p.get("PRECIO DE\nCOMPRA", 0) or 0)
        items.append((codigo, desc, precio))
...
def obtener_datos_productos():
    ...
    cods, descs, precios_local = load_local_products()
...
class MiWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self.productos_codigos, self.productos_descripciones,_ = obtener_datos_productos()
        self.mapping = {}
        self.mapping_inv = {}
